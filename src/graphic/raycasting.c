...
t_ray	*get_horizontal_collision(t_data *data)
{
	...
	while (!ray->coll)
	{
		...
		if (ray->y_end > array_len((void **)data->map))
			return (ray);
		if (data->map[(int)ray->y_end][(int)ray->x_end] == '1')
		{
			ray->coll = true ;
...
float	get_ray_len(t_data *data, t_ray *ray)
{
	...
	ray_len = (ray->x_end - data->player->x) / cos(ray->angle);
	return (ray_len);
}
...
void		draw_ray(t_data *data)
{
	...
	printf("x_end = %f\n", ray->x_end);
	printf("y_end = %f\n", ray->y_end);
	dda(data, ray->x_end + data->player->ray_coef_x, ray->y_end + data->player->ray_coef_y);
}
